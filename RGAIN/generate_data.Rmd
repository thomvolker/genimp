...
Now, to make the data a bit more complex, I also want it to include categorical variabales, so that we can also see how well the R implementation of GAIN does when it comes to categorical variables. 
```{r}
#laod packages
library(holodeck)
library(purrr)
...
# so now i have a mixed data set, but the way the categorical vairables are distributed is way to simple...
